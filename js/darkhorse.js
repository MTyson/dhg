  ...
  jQuery.fx.step.clip = function (fx) {
    ...
    if (fx.start && fx.end) {
      var sarr = new Array(),
        ...
        emOffset =
          fx.start[ss + 1] == "em"
            ? parseInt($(fx.elem).css("fontSize")) *
              1.333 *
              parseInt(fx.start[ss])
            : 1;
      ...
      fx.elem.style.clip =
        "rect(" +
        parseInt(fx.pos * (earr[0] - sarr[0]) + sarr[0]) +
        "px " +
        parseInt(fx.pos * (earr[1] - sarr[1]) + sarr[1]) +
        "px " +
        parseInt(fx.pos * (earr[2] - sarr[2]) + sarr[2]) +
        "px " +
        parseInt(fx.pos * (earr[3] - sarr[3]) + sarr[3]) +
        "px)";
    }
  };
})(jQuery);
...
var App = {
  ...
  attachNavigation: function () {
    ...
    if (!App.navAttached) {
      ...
      $("body").on("click", function (event) {
        if (
          $(event.target).hasClass("bg-div") ||
          ($(event.target).prop("tagname") &&
            $(event.target).prop("tagname").toLowerCase() == "body")
        ) {
          App.navigate();
        }
      });
  ...
  navigate: function (target) {
    ...
    $(".bg-div")
      .hide("clip", { direction: "vertical" }, 1000, "easeOutQuart")
      .promise()
      .then(function () {
        $(".intro").remove();
        $(".bg-div").remove();
  ...
  openPage: function (target) {
    ...
    $(".bottom-item").each(function (x, item) {
      // Make menu item a member of Page
  ...
  intro: function (initial) {
    ...
    var dhLogo = $(
      '<img src="images/dh-logo-2.jpg" style="max-height: 75vh; min-height:360px; display: none;" id="darkhorse-logo"/>'
  ...
  _initVideos: function (rootSelector) {
    $(rootSelector).each(function (count, el) {
      ...
      var player = videojs(id, {}, function () {
        ...
        var overlayText =
          videoEl.data("overlay") != null ? videoEl.data("overlay") : "";
...
Page.prototype._openAfterBgLoad = function () {
  ...
  $("#bg-div")
    ...
    .promise()
    .then(function () {
      this.css({ "min-height:": "100%", "min-width": "100%" }).css({
        height: "",
        width: ""
      }); // ensures bg resizes with page
      if (page.intro) {
        page.intro(this);
      } else {
...
Page.prototype._openAfterBlind = function () {
  ...
  if (page.html) {
    ...
    for (x in page.content) {
      var box = $("<div />")
        ...
        .addClass(page.contentClass)
        .prop("id", page.content[x].id)
        .css({
      ...
      if (page.content[x].image) {
        var img = $("<img />");
        var src =
          typeof page.content[x].image === "string"
            ? page.content[x].image
            : page.content[x].image.src;
        img.attr("src", src);
      ...
      if (page.content[x].video) {
        ...
        var video = $(
          '<video id="example_video_1" class="video-js vjs-default-skin" ' +
            ...
            page.content[x].video.width +
            '" height="' +
            page.content[x].video.height +
            '" ' +
            'poster="' +
            page.content[x].video.thumb +
            '"> ' +
            '<source src="' +
            page.content[x].video.src +
            '" type="video/webm"/> ' +
      ...
      if (page.content[x].content)
        box.append(
          "<div class='content'>" + page.content[x].content + "</div>"
        );
      if (page.button) {
  ...
  if (page.maxBoxes) {
    ...
    arrow.on("click", function (e) {
      ...
      if (!isLast) {
        $(".content-box.visible")
          .first()
          .hide("blind", { direction: "left" }, 600, "easeOutQuart")
          .removeClass("visible");
        $(".content-box")
          .eq(new Number(lastCount) + 1)
          .show("blind", { direction: "left" }, 600, "easeOutQuart")
          .addClass("visible");
      }
    });
  ...
  var boxSelector =
    ".content-box" + (page.contentClass ? "." + page.contentClass : "");
  $(boxSelector)
    .slice(0, page.maxBoxes ? page.maxBoxes : 1000)
    .each(function (x, e) {
      $(e).addClass("visible");
    });
...
Page.prototype.resize = function () {
  ...
  var boxPadding = 10,
    pagingArrows = 100;
  var boxW =
    viewW / this.boxCount - (boxPadding + this.boxCount) - pagingArrows;
};
var PageManager = {
...
var Pages = {
  ...
  overview: new Page({
    ...
    logos: [
      ...
      "images/pac-life-logo.png",
      "images/prolifics-logo.png",
      "images/deloitte-logo.png",
      "images/act-logo.png",
      "images/javaworld-logo.png",
      "images/dev-works-logo.png"
    ],
    logoIndex: 0,
    //"<div id='overview'>"+
    html:
      ...
      "<p>Everything on this plane began with <em>intention</em>.</p>" +
      ...
      "<p style='padding-bottom: 10px;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;of humanitarian activism.</p>" +
      "<div style='padding-bottom: 10px;'><p><span><b>We do.</b></span>  And so can you.</p></div>" +
      "<p>Experience a whole new energy relationship with yourself, your business and you rquest for success.</p>" +
      "<p>As recent as April of 2021 our writings have been syndicated around the world.</p>" +
    ...
    functions: [
      ...
      function () {
        ...
        setTimeout(function () {
          $("#overview-carousel").attr(
            "src",
            Pages["overview"].logos[Pages["overview"].logoIndex]
          );
          window.overviewCarouselInt = setInterval(function () {
            Pages["overview"].logoIndex >= Pages["overview"].logos.length - 1
              ? (Pages["overview"].logoIndex = 0)
              : Pages["overview"].logoIndex++;

            $("#overview-carousel").hide(
              "slide",
              { direction: "left" },
              250,
              function () {
  ...
  home: new Page({
    ...
    content: [
      {
        id: "expansion",
        image: "images/darkhorse-imagine.jpg",
        title: "EXPANSION",
        subtitle: "GREATER CLARITY",
        summary:
          "Our coaching and consultation services help bring new aspects of yourself and your image into sharper focus.",
        content:
          "By staying very present and listening to the desired outcome of every job, we quickly, elegently create a new dynamic source of energy for your project and your vision, leaving you proud, relieved and empowered."
      },
      {
        id: "development",
        image: "images/programmingdarkhorseweb1.jpg",
        title: "DEVELOPMENT",
        subtitle: "CREATE",
        summary:
          "Helping you decipher and discover, deeper drives and desires, helping you express yourself in the world and on the web.",
        content:
          "By staying very present and listening to the desired outcome of every job, we quickly, elegently create a new dynamic source of energy for your project and your vision, leaving you proud, relieved and empowered.<h3>Contact</h3>Contact: <a href='mailto:dev@darkhorse.tech'>dev@darkhorse.tech</a>"
      },
      {
        id: "articles",
        image: "images/darkhorsearticles-500.jpg",
        title: "ARTICLES",
        subtitle: "IN DEPTH",
        summary:
          "Read some of our articles that have been featured on the cover of JavaWorld and IBM's developerWorks.",
        content:
          "Below are tech articles written by the Lead Architect at Dark Horse" +
  ...
  contact: new Page({
    ...
    html:
      "<div id='contact'><!--<div style='text-align: left;'>Drop a Line</div>-->" +
...
$(document).ready(function () {
  ...
  for (var p in Pages) {
    menuHolder.append(
      $('<span class="bottom-item divider">' + p.toUpperCase() + "</span>")
    );
  }
